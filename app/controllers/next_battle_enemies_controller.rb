class NextBattleEnemiesController < ApplicationController
  ...
  def index
    ...
    @count	= NextBattleEnemy.notnil().includes(:enemy).search(params[:q]).result.hit_count()
    @search	= NextBattleEnemy.notnil().includes(:enemy).page(params[:page]).search(params[:q])
