class EmbryosController < ApplicationController
  ...
  def index
    ...
    @count	= Embryo.notnil().includes(:pc_name, :embryo).search(params[:q]).result.hit_count()
    @search	= Embryo.notnil().includes(:pc_name, :embryo).page(params[:page]).search(params[:q])
